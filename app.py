...
def jobs():
    try:
        # Get the user's field from session
        field = session.get('selected_field', '')
        ...
        return render_template("jobs.html", job_listings=job_listings, field=field)
        
    except Exception as e:
        print(f"Error in job search: {e}")
        return render_template("jobs.html", error=str(e))
